...
# Set up trajectory writer
dcd_writer = hoomd.write.DCD(filename='traj1.dcd', trigger=hoomd.trigger.Periodic(writing_interval),unwrap_full=True)
sim.operations.writers.append(dcd_writer)
gsd_writer = hoomd.write.GSD(filename='trajectory.gsd',trigger=hoomd.trigger.Periodic(writing_interval))
sim.operations.writers.append(gsd_writer)
...
def splitrun(steps,run_num):
    ...
    # Import initial condition
    sim.timestep=0 #timestep automatically accumulates over runs unless reset. Must be reset BEFORE setting a sim state.
    sim.create_state_from_gsd(filename="trajectory.gsd")

    # Set up trajectory writer
    dcd_writer = hoomd.write.DCD(filename='traj' + str(run_num) + '.dcd', trigger=hoomd.trigger.Periodic(writing_interval),unwrap_full=True)
    sim.operations.writers.append(dcd_writer)
    gsd_writer = hoomd.write.GSD(filename='trajectory.gsd',trigger=hoomd.trigger.Periodic(writing_interval))
    sim.operations.writers.append(gsd_writer)
